...
static const TComMv		diamondPattern[4] =
{ TComMv(-1,0),
	TComMv(0,1),
	TComMv(1,0),
	TComMv(0,-1)
...
static const TComMv		hexagonPattern[6] =
{	TComMv(-2,0),
	TComMv(-1,2),
	TComMv(1,2),
	TComMv(2,0),
...
Bool TEncOpenCL::checkError(cl_int error, const TChar * message)
{
	Bool success = true;
	if (error != CL_SUCCESS) 
	{
...
Void TEncOpenCL::checkErrorAndExit()
{
...
Bool TEncOpenCL::findDevice(Int device)
{
	...
	if (deviceFound)
	{
		...
		if (deviceFound)
		{
			...
			{
				deviceId = 0;
				printf("OpenCL ID device not found, use default GPU device \n");
			}
			...
			info = (TChar *)alloca(sizeof(TChar) * paramValueSize);  // String to display vendor name info
...
Bool TEncOpenCL::compileKernel(const TChar* fileName)
{
	Bool compileKernel = true;
	...
	kernelCalc = clCreateKernel(program, "calcSAD_AMP", &errNum);
...
Bool TEncOpenCL::createBuffers(UInt i_maxCtuWidth, UInt i_maxCtuHeight, Int i_searchRange)
{
	...
	searchPoints	= (TComMv*)xMalloc(TComMv, 40);
...
Void TEncOpenCL::calcMotionVectors(Pel* pelCtu, Pel* pelSearch, Int i_iRefStride, Int i_iCtuStride, Int i_areaSize, TComMv* pcMvSrchRngLT, Bool fastSearch, MEFastSearchMethod search)
{
	...
	if (fastSearch)
	{
		searchLenght = xCalcPoints(areaSize, searchPoints, search);
		for (int i = 0; i < searchLenght; i++)
		{
			Int x = searchPoints[i].getHor();
			Int y = searchPoints[i].getVer();
			...
			errNum |= clSetKernelArg(kernelCompare, 6, sizeof(Int), &x);
			errNum |= clSetKernelArg(kernelCompare, 7, sizeof(Int), &y);
		...
		minSadPred = (Distortion*)clEnqueueMapBuffer(queue, sadBuffer, CL_TRUE, CL_MAP_READ, 0, sizeof(Distortion) * NUM_CTU_PARTS, 0, NULL, NULL, &errNum);
		checkError(errNum, "clEnqueueMapBuffer - sadBuffer");

		ruiCostsPred = (Distortion*)clEnqueueMapBuffer(queue, ruiCostBuffer, CL_TRUE, CL_MAP_READ, 0, sizeof(Distortion) * NUM_CTU_PARTS, 0, NULL, NULL, &errNum);
		checkError(errNum, "clEnqueueMapBuffer - sadBuffer");

		XarrayPred = (Int*)clEnqueueMapBuffer(queue, XarrayBuffer, CL_TRUE, CL_MAP_READ, 0, sizeof(Int) * NUM_CTU_PARTS, 0, NULL, NULL, &errNum);
		checkError(errNum, "clEnqueueMapBuffer - XarrayBuffer");

		YarrayPred = (Int *)clEnqueueMapBuffer(queue, YarrayBuffer, CL_TRUE, CL_MAP_READ, 0, sizeof(Int) * NUM_CTU_PARTS, 0, NULL, NULL, &errNum);
...
Int TEncOpenCL::xCalcPoints(Int searchRange, TComMv *points, MEFastSearchMethod search)
{
	Int stride = 1;

	Int numPoints = 4;

	points[0] = diamondPattern[0];
	points[1] = diamondPattern[1];
	points[2] = diamondPattern[2];
	points[3] = diamondPattern[3];

	if (search)
	{
		for (Int i = 4; stride < searchRange; i+=6)
		{
			points[i]		= hexagonPattern[0].scaleMv(stride);
			points[i + 1]	= hexagonPattern[1].scaleMv(stride);
			points[i + 2]	= hexagonPattern[2].scaleMv(stride);
			points[i + 3]	= hexagonPattern[3].scaleMv(stride);
			points[i + 4]	= hexagonPattern[4].scaleMv(stride);
			points[i + 5]	= hexagonPattern[5].scaleMv(stride);
	...
	{
		for (Int i = 4; stride < searchRange; i += 4)
		{
			points[i]		= diamondPattern[0].scaleMv(stride);
			points[i + 1]	= diamondPattern[1].scaleMv(stride);
			points[i + 2]	= diamondPattern[2].scaleMv(stride);
			points[i + 3]	= diamondPattern[3].scaleMv(stride);
